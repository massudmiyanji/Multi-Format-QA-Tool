...
from main import  process_file,initialize_cache
...
def custom_retriever( docs: list[Document], use_compression: bool = False
) -> BaseRetriever:
    """Retriever to use."""
...
memory_instance = ConversationBufferMemory(memory_key='chat_history', return_messages=True, output_key='answer')

def costum_chain(retriever, use_flare: bool = True) -> ConversationalRetrievalChain:
    llm = LLM  # Replace with your LLM instance
    return ConversationalRetrievalChain.from_llm(
        llm=llm,
        retriever=retriever,
        memory=memory_instance,  # Use the global memory instance
        verbose=True,
...
def costum_retrieval_chain(
    ...
    for file in uploaded_files:
        ...
        docs.extend(process_file(temp_filepath))

    retriever = custom_retriever(docs=docs, use_compression=use_compression)
    chain = costum_chain(retriever=retriever, use_flare=use_flare)
